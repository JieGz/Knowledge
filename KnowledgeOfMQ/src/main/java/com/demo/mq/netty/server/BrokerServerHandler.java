...
public class BrokerServerHandler extends ChannelInboundHandlerAdapter {
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        ...
        System.out.println("服务器接收到来自客户端的消息：" + in);
        String sendStr;
        if ("CONSUME".equals(in)) {
            sendStr = Broker.consume();
        } else {
            Broker.produce(new Date().toString());
            sendStr = "消息队列已经接收到消息,并暂存起来了.";
        }
